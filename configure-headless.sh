...
# Auto-detect the actual Raspberry Pi model from the device tree -- this is
# what caused the accidental Pi4-run-as-Pi5 mistake before: nothing previously
# stopped you from picking the wrong option by hand. Detection drives the
# DEFAULT, but you can still override it if you genuinely have an untested
# combo (Pi4+V2.0 or Pi5+V1) -- this script only supports the two paired
# combinations below, so an override outside of them is unsupported.
_pi_model="$(tr -d '\0' < /proc/device-tree/model 2>/dev/null || echo "unknown")"
case "$_pi_model" in
    *"Raspberry Pi 4"*) _detected_default="pi4-v1"; _detected_label="Pi 4" ;;
    *"Raspberry Pi 5"*) _detected_default="pi5-v2"; _detected_label="Pi 5" ;;
    *) _detected_default=""; _detected_label="" ;;
esac

if [ -n "$_detected_default" ]; then
    success "Detected: $_detected_label ($_pi_model)"
else
    warn "Could not auto-detect Pi model from /proc/device-tree/model (\"$_pi_model\")."
    warn "Defaulting to Pi 5 + V2.0 -- override below if this isn't right."
fi
...
echo ""
_def="${HARDWARE_VARIANT:-${_detected_default:-pi5-v2}}"
while true; do
...
if [ -n "$_detected_default" ] && [ "$HARDWARE_VARIANT" != "$_detected_default" ]; then
    warn "You selected $HARDWARE_VARIANT, but this looks like a $_detected_label based on"
    warn "the actual hardware. Only proceed if you're intentionally testing an"
    warn "unsupported combination -- this is very likely a mistake otherwise."
...
# The upstream install.sh (run above) creates its own systemd service,
# seeed-voicecard.service, enabled at sysinit.target -- it runs on EVERY boot
# and unconditionally overwrites /etc/asound.conf with its own generic stock
# template, silently destroying the custom dmix/softvol config this script
# writes later in the "ALSA Shared Output" section. This was discovered the
# hard way: audio worked immediately after first configure, but broke silently
# on the very next reboot with zero indication why. Disable it permanently --
# we don't need its dynamic multi-board detection logic since this project
# only ever targets one known board per device, and the kernel module itself
# still loads fine via udev/DKMS without this service running.
if systemctl is-enabled --quiet seeed-voicecard.service 2>/dev/null; then
    info "Disabling upstream seeed-voicecard.service (overwrites asound.conf on every boot otherwise)..."
    sudo systemctl disable --now seeed-voicecard.service
    success "seeed-voicecard.service disabled -- our custom asound.conf will now persist across reboots."
fi
